...
int main(int argc, char** argv)
{
    ...
    string filters[] = {"-horizontal", "-vertical", "-blur", "-edge", "-greyscale", "-red", "green", "blue"};

    // Ensure proper usage
    if (argc < 3)
    {
        cerr << "Usage: ./main -filterOption inputfile.bmp\n";
        return 1;
    }

    // Validate input filter
    string filterOpt = argv[1];
    ...
    // Iterate over each row and add padding
    ...
    // Write new pixels to outFile and add padding
    for (int i = 0; i < height; i++)
    {
